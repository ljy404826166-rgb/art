...
runner 机器需要 Node.js 24、微信开发者工具、已登录且可以打开当前小程序项目。自动化 WebSocket 端口为 `9420`。为避免上一作业残留的 renderer 和 WebSocket 污染下一作业，工作流会先通过官方 CLI 退出 IDE，再以服务端口 `32070` 启动干净实例；随后等待 `9420` 就绪。启动、连接、冒烟和预览均有超时保护。该 runner 应视为专用测试资源，执行门禁时会重启其微信开发者工具。
...
在本任务收尾联调中，上一作业断开后 `9420` 仍可能保持监听，但对应 automation
会话已经失效。仅复用 TCP 端口会令下一次页面控制卡住。工作流因此改为每次作业先
`quit`，再 `auto` 启动干净实例，并显式等待 `9420` 就绪；该策略已在 runner 上实测。
