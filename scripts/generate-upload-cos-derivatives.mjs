...
async function main() {
  ...
  if (!options.run) {
    await writeFile(
      reportPath,
      `${JSON.stringify({ status: "dry-run", files: files.length, derivatives: DERIVATIVES.map((item) => item.kind) })}\n`,
      "utf8",
    );
  ...
  const results = await runPool(files, options.concurrency, async (filePath, index) => {
    ...
    try {
      ...
      if ((index + 1) % 25 === 0 || index === files.length - 1) {
        log(
          `Progress ${index + 1}/${files.length}; generated=${generated}, reused=${reused}, uploaded=${uploaded}, skipped=${skipped}, failed=${failed}`,
        );
      }
    } catch (error) {
      failed += 1;
  ...
  log(
    `Done. generated=${generated}, reused=${reused}, uploaded=${uploaded}, skipped=${skipped}, failed=${failed}`,
  );
