...
const SEARCH_FIELD_CONFIG = [
  ...
  {
    keys: ["artist", "artistDisplay", "artist_display", "artist_ids", "artist_labels"],
    weight: 110,
  },
...
function getFieldValue(item, keys) {
  return keys
    .flatMap((key) => toSearchTokens(item && item[key]))
    .filter(Boolean)
    .join(" ");
}
...
function getArtworkKey(item) {
  return (
    item &&
    (item._id || item.id || item.supabase_id || item.source_id || item.title || item.title_cn)
  );
}
...
function scoreArtwork(item, queryVariants) {
  ...
  SEARCH_FIELD_CONFIG.forEach((field) => {
    ...
    if (!fieldText) return;
    const bestFieldScore = queryVariants.reduce(
      (best, query) => Math.max(best, scoreField(fieldText, query)),
      0,
    );
